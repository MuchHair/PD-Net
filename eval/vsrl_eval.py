...
class VCOCOeval(object):
    ...
    # def _collect_detections_for_image_1(self, dets, image_id):
    #     # print('image_id sss')
    #     # import pdb
    #     # pdb.set_trace()
    #     det = dets[image_id]
    ...
    def _do_eval(self, detections_file, ovr_thresh=0.5, mode=0):

        vcocodb = self._get_vcocodb()
        # self._do_agent_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh)
        self._do_role_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh, eval_type='scenario_1',mode=mode)
        # self._do_role_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh, eval_type='scenario_2',mode=mode)

    def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.5, eval_type='scenario_1', mode=0):

        output_txt = detections_file[:len(detections_file) - 4] + '.txt'

        dets = io.load_pickle_object(detections_file)
        ...
        for i in range(len(vcocodb)):
            ...
            if mode == 1:
                pred_agents, pred_roles = self._collect_detections_for_image_1(dets, image_id)
            elif mode == 0:
            ...
            for aid in range(self.num_actions):
                ...
                for rid in range(len(self.roles[aid]) - 1):
                    ...
                    gt_role_inds = vcocodb[i]['gt_role_id'][gt_inds, aid, rid]  # index
                    gt_roles = -np.ones_like(gt_boxes)

                    for j in range(gt_boxes.shape[0]):  # person boxes.shape
                        if gt_role_inds[j] > -1:
                            gt_roles[j] = vcocodb[i]['boxes'][gt_role_inds[j]]  # role boxes index
