...
def draw_maskx(x1, y1, x2, y2, width, mask, modify=False):
    for padding in range(int(width)):
        r0 = x1 + padding
        r1 = x2 + padding
        rr, cc = skdraw.draw.line(int(y1), int(r0), int(y2), int(r1))

        for i in range(len(rr)) :
            if rr[i] >= mask.shape[0]:
                rr[i] = mask.shape[0] - 1
        for i in range(len(cc)):
            if cc[i] >= mask.shape[1]:
                cc[i] = mask.shape[1] - 1
...
def draw_masky(x1, y1, x2, y2, width, mask, modify=False):

    for padding in range(int(width)):
        r0 = y1 + padding
        r1 = y2 + padding
        rr, cc = skdraw.draw.line(int(r0), int(x1), int(r1), int(x2))
        for i in range(len(rr)):
            if rr[i] >= mask.shape[0]:
                rr[i] = mask.shape[0] - 1
        for i in range(len(cc)):
...
def vis_human_keypts(
        ...
        modify=False):
    LINKS =  [[16, 14], [14, 12], [17, 15], [15, 13], [12, 13], [6, 12], [7, 13], [6, 7], [6, 8],
    ...
    for i, j in LINKS:
        c0, r0, conf0 = keypts[i-1]
        c1, r1, conf1 = keypts[j-1]
...
def vis_human_keypts_openpose(
        ...
        line_color=(0, 255, 255),
        modify=False):
    LINKS =  [[0, 1], [1, 2], [2, 3], [3, 4], [1, 5], [5, 6], [6, 7], [0, 15], [15, 17],
              [0, 14], [14, 16], [1, 8], [8, 9], [9, 10], [1, 11], [11, 12], [12, 13], [8, 11]]
    if modify:
        img_ = img
    else:
        img_ = np.copy(img)
